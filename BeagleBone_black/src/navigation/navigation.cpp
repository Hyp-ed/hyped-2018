...
NavigationType Navigation::getBrakingDistance() const
{
  ...
  static constexpr std::array<NavigationType, 16> kCoeff_fast = {
  ...
  if (getVelocity() < 50.0) {
    ...
    for (unsigned int i = 0; i < kCoeff_slow.size(); ++i) {
      braking_distance += kCoeff_slow[i] * var;
      var *= norm_v;
    }
  } else {
    NavigationType norm_v = (getVelocity() - 41.4985) / 23.5436;
    for (unsigned int i = 0; i < kCoeff_fast.size(); ++i) {
      braking_distance += kCoeff_fast[i] * var;
      var *= norm_v;
    }
  }
